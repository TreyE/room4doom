...
impl CustomStatusbar {
    ...
    fn draw_ammo_big_pixels(&self, pixels: &mut impl PixelBuffer) {
        ...
        let mut start_x = self.keys[0].width as i32 + 2;

        let ammo = self.status.ammo[ammo as usize];

        if ammo > 99 {
            start_x += self.big_nums[0].width as i32 * 3;
        } else if ammo > 9 {
            start_x += self.big_nums[0].width as i32 * 2;
        } else {
    ...
    fn draw_face_pixels(&self, mut big: bool, upper: bool, pixels: &mut impl PixelBuffer) {
        ...
        let offset_x = (patch.width as i32) / 2 + patch.left_offset as i32;
...
impl SubsystemTrait for CustomStatusbar {
